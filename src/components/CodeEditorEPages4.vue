...
export default {
    ...
    setup(props) {
        ...
        const pageStates = ref({}); // Store EditorState for each page
        ...
        */
        ...
        const customFocusTheme = EditorView.theme({
            "&.cm-focused": {
        ...
        const initializeEditor = async (initialContent) => {
            const initialState = EditorState.create({
                ...
                extensions: [
                    basicSetup,
                    updatePlugin, // Now defined in setup
        ...
        const handleLocalSaveComplete = async () => {
            isDirty.value = false;
            let storedTimezone = await getTimezone();
            if (storedTimezone === null) {
                storedTimezone = 'UTC';
            ...
            lastSavedTime.value = dayjs().tz(storedTimezone || dayjs.tz.guess()).format('HH:mm:ss');
            syncStatus.value = 'synced'; // You might want to adjust this based on your sync logic
        }

        // Watch for page changes
        watch(() => props.currentPage, async (newPageId, oldPageId) => {
            if (oldPageId && isDirty.value) {
                // Save the state of the old page
                pageStates.value[oldPageId] = editorView.value.state;
            }
            ...
            try {
                ...
                calendarStore.$reset();
                // Always create a new state to ensure extensions are fully reinitialized
                const initialState = await initializeEditor(content);
        
                if (!editorView.value) {
                    createEditor(initialState);
                } else {
                ...
                // Store the new state
                pageStates.value[newPageId] = initialState;
                // Mark the page clean to start with
                isDirty.value = false;
        ...
        onMounted(async () => {
            syncManager.value.on('localSaveComplete', handleLocalSaveComplete);
        });
